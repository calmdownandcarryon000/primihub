...
namespace primihub {
  ...
  void Sh3Evaluator::init(u64 partyIdx, block prevSeed, block nextSeed, u64
                          buffSize) {
  ...
  void Sh3Evaluator::init(u64 partyIdx, CommPkg& comm, block seed,
                          u64 buffSize) {
  ...
  Sh3Task Sh3Evaluator::asyncMul(Sh3Task dependency, const si64& A,
                                 const si64& B, si64& C) {
  ...
  Sh3Task Sh3Evaluator::asyncMul(Sh3Task dependency, const si64Matrix& A,
    ...
    return dependency.then([&](CommPkgBase* comm, Sh3Task self) {
        ...
        auto comm_cast = dynamic_cast<CommPkg&>(*comm);
        comm_cast.mNext().asyncSendCopy(C.mShares[0].data(), C.mShares[0].size());
        auto fu = comm_cast.mPrev().asyncRecv(C.mShares[1].data(),
                                       C.mShares[1].size()).share();
  ...
  return dep.then([&](CommPkgBase* comm, Sh3Task self) {
    switch (mPartyIdx) {
      case 0: {
        ...
        self.then([
            fu1 = std::move(fu1),
            fu2 = std::move(fu2)] (CommPkgBase* comm, Sh3Task self) mutable {
                fu1.get();
                fu2.get();
            });
        break;
      ...
      case 1: {
        ...
        // share 1:
        auto fu2 = comm_cast.mNext().asyncRecv(c.mShares[1].data(),
                                        c.size()).share();
  ...
  Sh3Task Sh3Evaluator::asyncMul(Sh3Task dep, const i64& a,
      const sbMatrix& b, si64Matrix& c) {
    return dep.then([&](CommPkgBase* comm, Sh3Task self) {
      if (b.bitCount() != 1)
        throw RTE_LOC;
      ...
      switch (mPartyIdx) {
        case 0: {
          ...
          auto fu1 = comm_cast.mNext().asyncRecv(c.mShares[0].data(),
                      c.size()).share();
          auto fu2 = comm_cast.mPrev().asyncRecv(c.mShares[1].data(),
                      c.size()).share();
          self.then([fu1 = std::move(fu1), fu2 = std::move(fu2)](CommPkgBase* _,
                    Sh3Task __) mutable {
              fu1.get();
              fu2.get();
          });
        ...
        case 1: {
          ...
          i64* dd = c.mShares[0].data();
          auto fu1 = SharedOT::asyncRecv(comm_cast.mPrev(), comm_cast.mNext(), std::move(c0),
                        { dd, i64(c.size()) }).share();
        ...
        case 2: {
          ...
          i64* dd0 = c.mShares[1].data();
          auto fu1 = SharedOT::asyncRecv(comm_cast.mNext(), comm_cast.mPrev(), std::move(c0),
                        { dd0, i64(c.size()) }).share();
  ...
  TruncationPair Sh3Evaluator::getTruncationTuple(u64 xSize, u64 ySize, u64 d) {
    ...
    if (DEBUG_disable_randomization) {
      ...
      // if (mPartyIdx == 0)
      {
        ...
        mShareGen.mNextCommon.get(pair.mRTrunc[0].data(),
              pair.mRTrunc[0].size());
        mShareGen.mPrevCommon.get(pair.mRTrunc[1].data(),
              pair.mRTrunc[1].size());
  ...
  Sh3Task Sh3Evaluator::asyncMul(Sh3Task  dependency, const si64& A,
    ...
    return dependency.then([&, shift](CommPkgBase* comm, Sh3Task& self) -> void {
      ...
      auto& comm_cast = dynamic_cast<CommPkg&>(*comm);
      if (next < 2) comm_cast.mNext().asyncSendCopy(abMinusR);
      if (prev < 2) comm_cast.mPrev().asyncSendCopy(abMinusR);
  ...
  Sh3Task Sh3Evaluator::asyncMul(Sh3Task dependency, const si64Matrix& A,
    ...
    return dependency.then([&, shift](CommPkgBase* comm, Sh3Task& self) -> void {
      ...
      auto truncationTuple = getTruncationTuple(abMinusR.rows(),
                                                abMinusR.cols(), shift);
      ...
      if (rt.mPartyIdx < 2) {
        ...
        // perform the async receives
        auto fu0 = comm_cast.mNext().asyncRecv((*shares)[0].data(),
                    (*shares)[0].size()).share();
        auto fu1 = comm_cast.mPrev().asyncRecv((*shares)[1].data(),
                    (*shares)[1].size()).share();
