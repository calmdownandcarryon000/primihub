...
// 使用移步回调任务的方式将本地的i64类型数据转换为共享的si64类型数据
Sh3Task Sh3Encryptor::localInt(Sh3Task dep, i64 val, si64 & dest) {

  return dep.then([this, val, &dest](CommPkgBase* commPtr, Sh3Task& self) {
...
// 使用移步回调任务的方式从通信子获取一个si64类型的共享数据
...
Sh3Task Sh3Encryptor::localBinary(Sh3Task dep, i64 val, sb64 & ret) {
  return dep.then([this, val, &ret](CommPkgBase* commPtr, Sh3Task& self) {
    ret[0] = mShareGen.getBinaryShare() ^ val;
    auto comm_cast = dynamic_cast<CommPkg&>(*commPtr);
...
Sh3Task Sh3Encryptor::remoteIntMatrix(Sh3Task dep, si64Matrix & ret) {
  return dep.then([this, &ret](CommPkgBase* commPtr, Sh3Task& self) {
    for (i64 i = 0; i < ret.mShares[0].size(); ++i)
        ret.mShares[0](i) = mShareGen.getShare();
    auto comm_cast = dynamic_cast<CommPkg&>(*commPtr);
    comm_cast.mNext().asyncSendCopy(ret.mShares[0].data(), ret.mShares[0].size());
...
Sh3Task Sh3Encryptor::localBinMatrix(Sh3Task dep, const i64Matrix & m,
  ...
  return dep.then([this, &m, &ret](CommPkgBase* commPtr, Sh3Task self) {
    ...
    self.then([fu = std::move(fu)](CommPkgBase* commPtr, Sh3Task& self) mutable {
        fu.get();
    });
  }).getClosure();
}
...
Sh3Task Sh3Encryptor::remoteBinMatrix(Sh3Task dep, sbMatrix & ret) {
  return dep.then([this, &ret](CommPkgBase* comm, Sh3Task& self) mutable {
    for (u64 i = 0; i < ret.mShares[0].size(); ++i)
      ret.mShares[0](i) = mShareGen.getBinaryShare();
...
Sh3Task Sh3Encryptor::localPackedBinary(Sh3Task dep, MatrixView<u8> m,
  ...
  return dep.then([this, m, &dest, transpose](CommPkgBase* comm, Sh3Task& self) {
    ...
    auto comm_cast = dynamic_cast<CommPkg&>(*comm);
    comm_cast.mNext().asyncSendCopy(dest.mShares[0].data(), dest.mShares[0].size());
...
Sh3Task Sh3Encryptor::remotePackedBinary(Sh3Task dep, sPackedBin & dest) {
  return dep.then([this, &dest](CommPkgBase* comm, Sh3Task& self) {
    for (u64 i = 0; i < dest.mShares[0].size(); ++i)
      dest.mShares[0](i) = mShareGen.getBinaryShare();
    auto comm_cast = dynamic_cast<CommPkg&>(*comm);
    comm_cast.mNext().asyncSendCopy(dest.mShares[0].data(), dest.mShares[0].size());
    ...
    self.then(std::move([fu = std::move(fu)](CommPkgBase* comm, Sh3Task& self)
                  mutable {
        fu.get();
    }));
  }).getClosure();
...
Sh3Task Sh3Encryptor::reveal(Sh3Task dep, const sbMatrix& x,
                              i64Matrix& dest) {
  return dep.then([&x, &dest](CommPkgBase* comm, Sh3Task& self) {
    auto comm_cast = dynamic_cast<CommPkg&>(*comm);
    comm_cast.mNext().recv(dest.data(), dest.size());
...
void Sh3Encryptor::reveal(CommPkg & comm, const si64Matrix & x,
                          i64Matrix & dest) {
...
void Sh3Encryptor::reveal(CommPkg & comm, u64 partyIdx,
                          const si64Matrix & x) {
  if ((mPartyIdx + 2) % 3 == partyIdx)
    comm.mPrev().asyncSendCopy(x.mShares[0].data(), x.mShares[0].size());
}

void Sh3Encryptor::reveal(CommPkg & comm, const sbMatrix & x,
                          i64Matrix & dest) {
  if (dest.rows() != static_cast<i64>(x.rows()) ||
...
void Sh3Encryptor::reveal(CommPkg & comm, const sPackedBin & A,
                          i64Matrix & r) {
  auto wordWidth = (A.bitCount() + 8 * sizeof(i64) - 1) / (8 * sizeof(i64));
  i64Matrix buff;
  buff.resize(A.bitCount(), A.simdWidth());
...
Sh3Task Sh3Encryptor::reveal(Sh3Task dep, const sPackedBin & A,
                              PackedBin & r) {
  return dep.then([&A, &r](CommPkgBase* comm, Sh3Task&  self) {
...
void Sh3Encryptor::revealAll(CommPkg & comm, const sPackedBin & A,
                              i64Matrix & r) {
