...
struct sf64Matrix : private si64Matrix {
  ...
  si64Matrix& i64Cast() {
    return static_cast<si64Matrix&>(*this);
  }
...
template<Decimal D>
inline const typename sf64Matrix<D>::Row& sf64Matrix<D>::Row::operator=(
  const Row & row) {
...
template<Decimal D>
inline const typename sf64Matrix<D>::ConstRow& sf64Matrix<D>::Row::operator=(
  const ConstRow & row) {
  mMtx.mShares[0].row(mIdx) = row.mMtx.mShares[0].row(row.mIdx);
  mMtx.mShares[1].row(mIdx) = row.mMtx.mShares[1].row(row.mIdx);
  return row;
...
template<Decimal D>
inline const typename sf64Matrix<D>::Col& sf64Matrix<D>::Col::operator=(
  const Col & row) {
...
template<Decimal D>
inline const typename sf64Matrix<D>::ConstCol& sf64Matrix<D>::Col::operator=(
  const ConstCol & row) {
  mMtx.mShares[0].col(mIdx) = row.mMtx.mShares[0].col(row.mIdx);
  mMtx.mShares[1].col(mIdx) = row.mMtx.mShares[1].col(row.mIdx);
  return row;
